/*
 ...
 Date: 07/11/2022 15:50:58
*/


-- ----------------------------
-- Table structure for rn_db_ocv1
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[rn_db_ocv1]') AND type IN ('U'))
	DROP TABLE [dbo].[rn_db_ocv1]
GO

CREATE TABLE [dbo].[rn_db_ocv1] (
...
ALTER TABLE [dbo].[rn_db_ocv1] SET (LOCK_ESCALATION = TABLE)
GO


-- ----------------------------
-- Auto increment value for rn_db_ocv1
-- ----------------------------
DBCC CHECKIDENT ('[dbo].[rn_db_ocv1]', RESEED, 2058662)
GO


-- ----------------------------
-- Primary Key structure for table rn_db_ocv1
-- ----------------------------
ALTER TABLE [dbo].[rn_db_ocv1] ADD CONSTRAINT [PK_rn_db_ocv1] PRIMARY KEY CLUSTERED ([ID])
